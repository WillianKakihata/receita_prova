...
export class IngredientService {
  ...
  public async create(createIngredient: Ingredient): Promise<Ingredient> {
    try {
      const ingredient =
        await this.ingredientRepository.create(createIngredient);
  ...
  public async update(id: string, updateItem: Ingredient) {
    try {
      return await this.ingredientRepository
        .findByIdAndUpdate(id, updateItem, {
          new: true,
        })
        .exec();
    } catch (error) {
  ...
  public async deleteMany(ids: Types.ObjectId[]) {
    try {
      return this.ingredientRepository
        .deleteMany({ _id: { $in: ids.map((id) => id.toString()) } })
        .exec();
    } catch (error) {
      throw new BadRequestException(error);
    }
  ...
  public async removeIngredients(id: string, idIngredient: string) {
    try {
      const remove = await this.recipeRepository.findByIdAndUpdate(id, {
  ...
  public async receiveIdRecipe(id: string, idRecipe: string) {
    try {
      ...
      return await this.ingredientRepository.findByIdAndUpdate(id, {
